...
function cleanIcons() {
  return del(['src/modules/dhug_icons.module/module.html']);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    src(file).pipe(
      through.obj((file, enc, cb) => {
        ...
        exec(
          `yarn hs fetch --portal=prod dhug/modules/${dir}/meta.json ${localPath} --overwrite`,
          (err, stdout, stderr) => {
            console.log(stdout);
            resolve();
            cb(err);
  ...
  return new Promise((resolve, reject) => {
    src(file).pipe(
      through.obj((file, enc, cb) => {
        ...
        exec(
          `yarn hs fetch --portal=prod dhug/modules/${dir}/fields.json ${file.path} --overwrite`,
          (err, stdout, stderr) => {
            console.log(stdout);
            resolve();
...
function buildTemplates() {
  console.log('Syncing templates to portal');
  return src([
    'src/templates/**/*',
...
const Watch = function () {
  watch(['src/icons/*.svg', '!src/icons/icons.svg'], BuildIcons);
